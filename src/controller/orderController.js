...
const createOrder = async function (req, res) {
    try {
        ...
        let { items, totalPrice, totalItems, cancellable, status } = req.body
...
const updateOrder = async function (req, res) {
    try {
        ...
        if (!(order.status == "pending")) {
            return res.status(400).send({ status: false, msg: "cannot change status of already completed  or cancle order" })
        }
